    ...
    def _signal_handler(self, signum, frame):
        ...
        with self._lock:
            if not self.shutdown_requested:
                logger.info(f"Received exit signal ({signum}); waiting for the current task to finish...")
...
class Scheduler:
    ...
    def __init__(
        ...
        try:
            ...
            raise ImportError("Please install the schedule library: pip install schedule")
    ...
    def set_daily_task(self, task: Callable, run_immediately: bool = True):
        ...
        if not self._configure_daily_tasks(self.schedule_times):
            raise ValueError(f"Invalid schedule time: {self.schedule_time!r}")

        if run_immediately:
            logger.info("Running the task once immediately...")
            self._safe_run_task()
    ...
    def _configure_daily_task(self, schedule_time: str) -> bool:
        ...
        if not self._is_valid_schedule_time(candidate):
            logger.warning(
                "Detected invalid schedule time %r; keeping current time %s",
                schedule_time,
                self.schedule_time,
            )
            return False
        ...
        if previous_time == candidate:
            logger.info("Daily scheduled task configured, run time: %s", self.schedule_time)
        else:
            logger.info(
                "Detected SCHEDULE_TIME change; daily scheduled task updated from %s to %s",
                previous_time,
                self.schedule_time,
            )
        return True
    ...
    def _refresh_daily_schedule_if_needed(self) -> None:
        ...
        try:
            ...
            logger.warning("Failed to read latest SCHEDULE_TIME, keeping %s: %s", self.schedule_time, exc)
            return
        ...
        if self._configure_daily_task(latest_schedule_time):
            logger.info("Updated next run time: %s", self._get_next_run_time())
    ...
    def _safe_run_task(self):
        ...
        try:
            ...
            logger.info(f"Scheduled task finished - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    def add_background_task(
        ...
        if int(interval_seconds) < 30:
            logger.warning(
                "Background task %s requested a %ds interval, but the scheduler loop polls every 30s; adjusted to 30s",
        ...
        logger.info(
            "Registered background task: %s (interval %s s, run_immediately=%s)",
            entry["name"],
    ...
    def run(self):
        ...
        logger.info(f"Next run time: {self._get_next_run_time()}")
        ...
        while self._running and not self.shutdown_handler.should_shutdown:
            ...
            if datetime.now().minute == 0 and datetime.now().second < 30:
                logger.info(f"Scheduler running... next run: {self._get_next_run_time()}")
...
if __name__ == "__main__":
    ...
    print("Starting test scheduler (press Ctrl+C to exit)")
