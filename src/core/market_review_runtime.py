...
def build_market_review_runtime(
    ...
    if has_configured_llm_runtime(config):
        ...
        if backend_error is not None:
            logger.error("AI analyzer generation backend configuration error: %s", backend_error)
        elif not analyzer.is_available():
            logger.warning("AI analyzer unavailable after initialization; please check the LLM configuration")
            analyzer = None
    else:
        ...
        if backend_error is not None:
            ...
            logger.warning("No LLM model configuration detected; reports will be generated from templates only")
