...
def _is_windows_process_alive(pid: int) -> bool:
    ...
    try:
        ...
        logger.warning("Windows process liveness check failed; conservatively treating it as still running: %s", exc)
        return True
        ...
        if fcntl is not None:
            ...
            for _ in range(2):
                try:
                    ...
                    logger.warning("Stale market_review.lock detected; cleaning up and retrying.")
                    try:
                        lock_path.unlink()
                    except OSError as exc:
                        logger.warning("Failed to clean up stale market_review.lock: %s", exc)
                        return None
