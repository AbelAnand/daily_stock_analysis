    ...
    *,
    ...
    for candidate_model in model_chain:
        ...
        for attempt in range(max_retries + 1):
            ...
            if attempt:
                attempt_prompt += (
                    "\n\nThe previous output did not meet the structured coverage requirement. "
                    "Return strict JSON again and cover as many candidate codes as possible."
                )
            try:
...
def _build_ranking_prompt(
    ...
    *,
    ...
    context_text = context.strip() or "No additional context. Judge only from the structured candidate data and strategy preferences."
...
def _render_ranking_prompt(hints: str, context: str, candidates_text: str) -> str:
    return f"""You are a professional equity research analyst. Your task is to produce a relative ranking within a candidate pool that has already been hard-filtered by code.
You must not recommend stocks outside the pool, change the hard-filter conditions, or give price targets or promised returns. Your value lies in:
1. Comparing candidates against each other in light of the strategy preferences;
2. Identifying potential catalysts, style fit and risk points that the structured data does not expose;
3. Attributing industry/concept heat and the DSA-supplied quotes, fundamentals and news semantically, without treating single-day heat as the sole buy reason;
4. Giving short, auditable, verifiable ranking rationale.
...
Format:
{{
  "market_view": "One sentence on whether the current candidate pool and market backdrop suit this strategy",
  "selection_logic": "The 2-3 main judgment dimensions used in this ranking",
  ...
  "ranked": [
    {{
      ...
      "llm_score": 0-100,
      "confidence": 0-1,
      "sector": "Short industry/theme label; prefer the candidate's industry/concepts and keep labels consistent, e.g. brokers, banks, pharma, AI compute",
      "theme": "Main trading logic or theme",
      "thesis": "Core investment hypothesis for this candidate",
      "reason": "One-sentence ranking rationale",
      "risk": "One-sentence main risk",
      "catalysts": ["potential catalyst 1", "potential catalyst 2"],
      "risk_flags": ["risk tag 1"],
      "tags": ["value", "trend", "defensive", "event", "liquidity"],
      "style_fit": "How well it matches the strategy style",
      "watch_items": ["data or events to track next"],
      "invalidators": ["observations that would invalidate the thesis"]
    }}
  ]
}}
"""
