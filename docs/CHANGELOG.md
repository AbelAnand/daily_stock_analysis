...
## [Unreleased]
- [改进] Web settings API now masks every sensitive field (API keys, tokens, secrets, passwords) at read time instead of only the Hermes / telemetry keys; saving a masked value keeps the stored secret unchanged, and the raw .env export stays gated behind admin auth.
- [新功能] Paper-trading dashboard: new Web "Trading" page (/trading) with live account equity, bot P&L since its first trade, day / unrealized P&L, open positions with protective stop, target and R multiple, open orders, recent activity, a cumulative bot P&L chart anchored at the bot's first trade, a per-position Close button and a typed-confirmation Close-all; backed by GET /api/v1/paper-trading/dashboard and POST /api/v1/paper-trading/positions/{symbol}/close | /close-all, with every manual close recorded in paper_trades (docs/paper-trading.md).
...
- [新功能] Trade learning loop (opt-in, TRADE_POSTMORTEM_ENABLED): every closed paper trade gets an LLM post-mortem (process classification + at most one distilled lesson); recent lessons are injected into the analyst prompt as calibration (never a veto), execution flaws are escalated to the digest for engineering instead, and the daily digest gains a strategy scorecard by source × direction (docs/trade-learning.md).
- [新功能] News-driven discovery (opt-in, DISCOVERY_ENABLED): the daily run scans Alpaca news + movers/most-actives, triages catalysts with a cheap LLM, and merges up to DISCOVERY_MAX_CANDIDATES vetted US symbols into the analysis run (docs/news-discovery.md); discovery failure never touches the configured list.
- [新功能] Short selling (opt-in, PAPER_TRADING_ALLOW_SHORT): bearish sell/avoid signals may carry an ev_contract.short_plan (stop above entry, target below); valid plans execute as short brackets with mirrored R:R gates, chase-down, shortable/ETB check, cover-on-bullish-flip, and a mirrored stop ratchet; outcome scoring rates such signals against direction "down".
- [修复] Paper trading: new entries are no longer submitted while the market is closed — a pre-market marketable order can fill through its own stop on an opening gap (observed live). Entries are deferred to the intraday pass (09:32 ET+), which prices them off live quotes; max one entry per symbol per day.
- [新功能] Paper trading: open positions are now managed after entry — once a position is up PAPER_TRADING_BREAKEVEN_TRIGGER_R (default 1R) the bracket stop is raised in place to break-even (+PAPER_TRADING_BREAKEVEN_BUFFER_PCT) and then trails the price by PAPER_TRADING_TRAIL_R; never lowered. Runs after the daily step and standalone via `python main.py --manage-positions` for an intraday timer (see docs/paper-trading.md).
- [改进] Paper trading: new entries get an ATR stop-distance floor — a plan stop closer than PAPER_TRADING_STOP_MIN_ATR_MULT × ATR(14) (default 1.0) to the entry is widened to that distance before the R:R gates and sizing run, so noise-level stops are either given room or the plan is rejected; ATR comes from the signal's source report, else from broker daily bars (docs/paper-trading.md).
- [改进] Trade learning loop: post-mortems now also run in the intraday `--manage-positions` pass while the market is open, so a stop-out is reviewed the same day and its lesson reaches the next morning's analysis.
