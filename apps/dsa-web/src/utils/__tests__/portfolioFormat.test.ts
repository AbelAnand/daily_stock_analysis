...
describe('portfolioFormat', () => {
  ...
  it('formats position price fields based on price availability', () => {
    ...
    expect(getPositionPriceLabel(pricedPosition, 'zh')).toBe('实时价 · longbridge');
    expect(getPositionPriceLabel(pricedPosition, 'en')).toBe('Realtime · longbridge');
    ...
    expect(getPositionPriceLabel(missingPosition, 'zh')).toBe('缺价');
    expect(getPositionPriceLabel(missingPosition, 'en')).toBe('Price unavailable');
  });

  it('formats broker labels and CSV result variants', () => {
    expect(formatBrokerLabel('huatai', undefined, 'zh')).toBe('huatai（华泰）');
    expect(formatBrokerLabel('huatai', undefined, 'en')).toBe('huatai（Huatai）');
    expect(formatBrokerLabel('custom', ' 自定义 ', 'zh')).toBe('custom（自定义）');
